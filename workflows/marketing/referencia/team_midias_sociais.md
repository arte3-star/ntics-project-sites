...
- Gmail MCP conectado (para drafts de newsletter) ou Brevo API (`BREVO_API_KEY`) para envio em massa
...
| **Publisher** | `publisher` | Monta newsletter HTML, cria drafts Gmail/Brevo, cria tasks ClickUp | Gmail MCP, Brevo API, ClickUp MCP |
...
2. TEXTO LINKEDIN (post/carrossel/artigo):
   - OBRIGATORIO: ler workflows/marketing/referencia/linkedin_strategy.md antes de escrever
   - Tom: 70% formal, 65% tecnico, 75% dados
   - Estrutura post carrossel projeto: gancho com dado → contexto → resultados (bullets) → CTA consultivo
   - Estrutura post carrossel noticias: insight tendencia → lista numerada → reflexao → pergunta engajadora
   - Estrutura artigo: titulo com dado concreto (max 60 chars) → corpo adaptado (mais dados, menos analogias)
   - Mencionar NTICS Projetos, lei de incentivo relevante, ODS
   - Sempre tagar empresa patrocinadora (@Empresa)
   - Max 1.300 chars para post, artigo sem limite
   - Hashtags: MAXIMO 5 (#ImpactoSocial #ESG #LeiDeIncentivo #Sustentabilidade #ODS)
   - Sem emojis excessivos (max 1-2, nunca no gancho)
   - CTA consultivo ("Fale com nosso time", "Conheca o case") — nunca "salve/compartilhe"

3. EMAIL / NEWSLETTER:
   - Seguir estrutura do workflow workflows/marketing/producao/newsletter.md
...
4. SCRIPT NARRACAO (NotebookLM):
   ...
   - Referencia: workflows/marketing/producao/roteiro_video.md
...
1. NEWSLETTER HTML + DRAFT:
   - Workflow: workflows/marketing/producao/newsletter.md
   - Canal Gmail: mcp__claude_ai_Gmail__gmail_create_draft (testes/internos)
   - Canal Brevo: python tools/publishing/publish_to_brevo.py (envio em massa)
   ...
   - Crie draft no Gmail ou campanha draft no Brevo
...
3. LINKEDIN — CARROSSEL PDF + POST:
   - Converter cards JPG em PDF unico para upload como documento LinkedIn
   - Usar caption LinkedIn do Content Writer (.tmp/copy/linkedin-post-*.txt)
   - Para artigos mensais: publicar como LinkedIn Article nativo
   - Referencia: workflows/marketing/referencia/linkedin_strategy.md
   - REGRAS LINKEDIN:
     * Formato: documento PDF (carrossel) ou Article (artigo mensal)
     * NAO postar videos
     * Hashtags: maximo 5 por post
     * Sempre tagar empresa patrocinadora
     * Frequencia: max 2x/semana (Terca e Quinta, 8h-10h)

4. ORGANIZACAO DE ASSETS:
   - Monte pacote final por plataforma em .tmp/publicacao/:
     ...
     .tmp/publicacao/
       ...
       ├── linkedin/
       │   ├── carrossel.pdf    # PDF com todos os cards (documento)
       │   ├── post.txt         # caption LinkedIn (tom analitico/consultivo)
       │   └── article.md       # artigo mensal adaptado (quando aplicavel)
       ├── newsletter/
...
4. Para LinkedIn: converter cards em PDF e usar caption LinkedIn especifica
5. Crie draft Gmail para newsletter
6. Crie tasks no ClickUp para cada publicacao pendente
7. Envie mensagem ao Lead com resumo: o que foi criado, onde esta, proximos passos
```
...
- Plano mensal vigente (se existir, `workflows/marketing/producao/plano_mensal.md`)
