# Workflow: Carrossel de Projeto Ativo do Cliente
...
## Objetivo
Gerar carrossel pronto para Instagram (8 cards: capa + 6 conteudo + CTA) a partir de release, TAP ou briefing de projeto ativo (pre-projeto ou durante), usando a identidade visual do cliente/patrocinador.
...
**Opcao A — Fotos manuais (usuario fornece):**

Pedir ao usuario para colocar 6 fotos na pasta:
```
output/marketing/carrosseis/clientes/{numero-projeto}/{fase}/fotos/
```
...
**Opcao B — Extracao contextual do relatorio PDF (quando o projeto tem relatorio com fotos):**

Se o usuario tem um relatorio PDF do projeto com fotos embutidas, usar a ferramenta de extracao contextual:

```bash
python tools/media/extrair_fotos_relatorio.py \
  --pdf ".tmp/marketing/carrosseis/{pasta-projeto}/relatorio.pdf" \
  --output ".tmp/marketing/carrosseis/{pasta-projeto}/fotos-contextuais" \
  --min-size 1000 \
  --top-n 3
```

A ferramenta:
1. Le o texto de cada pagina do PDF para entender o contexto
2. Identifica quais paginas sao mais relevantes para cada card (capa, o_projeto, metodologia, alcance, a_empresa, resultados, impacto)
3. Extrai apenas as imagens dessas paginas, filtrando logos e barras decorativas
4. Gera `fotos-contextuais/manifest.json` com o mapeamento card → fotos rankeadas

**Fallback automatico:** Para cada card, o manifest tem rank 1 (principal), rank 2 e rank 3 (fallbacks). Se a foto principal nao ficar boa na geracao do Leonardo AI, trocar para o proximo rank e regenerar so aquele card.
...
### Passo 8 — Revisao Visual Automatica (OBRIGATORIA antes de apresentar ao usuario)

O Leonardo AI frequentemente introduz erros de texto nas imagens geradas. Esta fase e BLOQUEANTE — nenhum card pode ser apresentado ao usuario sem passar por esta revisao completa.

**Procedimento:**

1. **Abrir cada card gerado** e ler TODO o texto visivel na imagem
2. **Comparar com o prompt original** palavra por palavra
3. **Registrar erros** encontrados numa lista
4. **Regenerar** cards com erro (loop ate passar)

**Checklist por card:**
- [ ] Texto sem palavras duplicadas (ex: "COM A AGENDA COM A AGENDA" — erro comum do Leonardo)
- [ ] Texto sem palavras cortadas ou faltando
- [ ] Acentos corretos em portugues (Educação, não EDUCACAO; Espetáculos, não ESPETACULOS)
- [ ] Numeros identicos ao release/TAP/briefing (sem trocar digitos)
- [ ] Sem "porcento" — usar "%" ou "por cento"
...
**Erros mais frequentes do Leonardo AI (aprendizados):**

| Tipo de erro | Exemplo | Causa provavel |
|---|---|---|
| Palavra duplicada | "NO NO MATO GROSSO" | Prompt com frase longa, modelo repete tokens |
| Trecho duplicado | "COM A AGENDA COM A AGENDA 2030" | Headline longa com highlight, modelo duplica bloco |
| Acento faltando | "EDUCACAO" em vez de "EDUCAÇÃO" | Modelo ignora acentos em uppercase — forcar no prompt |
| Formato numerico | "porcento" em vez de "%" | Prompt usou "porcento" por extenso |
...
**Dicas para evitar erros na regeneracao:**
- Encurtar headlines longas (maximo ~8-10 palavras)
- Nao repetir palavras no titulo e no corpo do prompt
- Forcar acentos no prompt: escrever "Educação" em vez de "Educacao"
- Usar "%" em vez de "porcento"
- Se o erro persistir apos 2 tentativas, reformular completamente a frase
...
**Estrutura da pasta:**
```
output/marketing/carrosseis/cases/{slug-projeto}/
├── 01-capa.jpg
├── 02-o-projeto.jpg
├── 03-a-empresa.jpg
├── 04-alcance.jpg
