...
Roda automaticamente no SessionStart do Claude Code em Claude-NTICS-Projetos. Puxa estado do ClickUp, diffa contra cache, só invoca LLM (Haiku) quando há delta real.

## O que faz

1. Lê `SecondBrain/projetos/{slug}/tasks.yaml` → pega `task_mae.id` (ex: 868j96x17 para 132)
2. Puxa via ClickUp API: task-mãe + subtasks + comentários
3. Compara contra `SecondBrain/projetos/{slug}/_cache/clickup-snapshot.json`
...
6. Escreve eventos em `SecondBrain/projetos/{slug}/comms/events.log`
...
python tools/sync/projeto_sync.py 132-estacao-samarco --json --quiet
```
...
## Custo real esperado (132-estacao-samarco)
...
Com SessionStart (abrindo Claude-NTICS-Projetos ~5 vezes/dia): **<$1/mês** na fase atual do projeto.
...
O script puxa threads Gmail cuja query está definida em `SecondBrain/projetos/{slug}/tasks.yaml` no campo `gmail_query`. Usa o auth OAuth já configurado em `AUTOMAÇÕES/tools/gws/token.json` (via `gws_auth.get_credentials()`).
...
Isso registra uma task Windows que roda a cada 120 min. Logs em `SecondBrain/projetos/132-estacao-samarco/_cache/cron.log`.

Para remover:
```powershell
schtasks /Delete /TN "NTICS-projeto-sync-132-estacao-samarco" /F
```
...
- **Decisões capturadas vão para SecondBrain:** já implementado — append em `SecondBrain/projetos/{slug}/decisoes.md` e `execucao.md`.
- **Telemetria de custo:** logar tokens consumidos Haiku em `_cache/cost.log` para monitorar se o custo estimado ($3-8/mês) bate com a realidade.
