...
def fetch_story_image(story: dict, unsplash_key: str, output_path: Path) -> dict:
    ...
    # Fallback intermediário: Google CSE (se configurado)
    google_api_key = os.getenv("GOOGLE_CSE_API_KEY")
    google_cse_id = os.getenv("GOOGLE_CSE_ID")
    if google_api_key and google_cse_id:
        print(f"  Unsplash found nothing — trying Google CSE...")
        sys.path.insert(0, str(Path(__file__).parent))
        from fetch_images_google import search_and_download as _google_search
        google_result = _google_search(query, google_api_key, google_cse_id, output_path)
        if google_result:
            print(f"  ✓ Google CSE: {google_result['image_url'][:80]}...")
            return {"source": "google", **google_result}
        print(f"  Google CSE found nothing — falling back to Leonardo AI")
