...
ASSIGNEE PRIORITY (apply in order — AVOID null, it is the last resort):
1. EXPLICIT NAME in the task context/excerpt → use that person's ClickUp ID
   Examples: "Ângelo vai enviar o release" → 81549824; "Luisa cria o carrossel" → 87399549
2. MEETING TITLE contains "Coord. X" or "Sprint X" pattern → that coordinator is the default
   for any task in this meeting where no explicit name is mentioned
   Examples: "Sprint projetos - Coord. Mayara" → default 81460588; "Sprint AI/Comunicação" → default 81513300
3. KEYWORD RULES (fallback when no name and no title pattern):
   - produção/campo/evento/execução → 81460587 (Raíza) or 81477520 (Jessica)
   - financeiro/NF/nota fiscal/pagamento/reembolso/orçamento → 126152139 (Vera)
   - jurídico/contrato/compliance/acordo → 81460586 (Luiz Felipe)
   - vendas/proposta/captação/negociação/cliente → 81470291 (Abilio) or 81502630 (Cristina)
   - comunicação/IA/site/post/automação/redes sociais/conteúdo → 81513300 (Lucas)
   - inscrição/MINC/prestação de contas/auditoria → 81460588 (Mayara)
   - coordenação/prazo/PMO/cronograma/entrega → 81461902 (Bruna)
   - assessoria/release/clipping/imprensa/press → 81549824 (Angelo)
   - design/carrossel/visual/identidade/arte → 87399549 (Luisa)
   - revisão/aprovação/decisão estratégica → 81513300 (Lucas) or 81470291 (Abilio)
4. null → ONLY if absolutely none of the above applies
...
{transcript[:12000]}"""
    ...
    # All tasks always go to the single triage/tasks list
    result["list_id"] = "901113425673"

    # Apply low-confidence fallback
    if result.get("confidence", 0) < 80:
        print(f"⚠️  Low confidence ({result['confidence']}%) — marking for triage review")
        result["triage"] = True
